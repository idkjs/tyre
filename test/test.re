module A = {
  ...
  let choice =
      ...
      : (module TESTABLE with type t = [ | `Left(M1.t) | `Right(M2.t)]) =>
    (module
     {
       type t = [ | `Left(M1.t) | `Right(M2.t)];
       let pp = ppf =>
         fun
         | `Left(x) => M1.pp(ppf, x)
         | `Right(x) => M2.pp(ppf, x);
...
exception ConvFail;
let cfail: t(unit) = (
  conv(_ => raise(ConvFail), _ => raise(ConvFail), regex(Re.any)): t(unit)
);
...
let convfail = (title, desc, re, s) => (
  ...
  () => {
    ...
    A.check_raises(title ++ " all_seq", ConvFail, () =>
...
let composed = [
  ...
  t(
    ...
    [`Left("1"), `Right("a"), `Left("2"), `Left("5"), `Right("c")],
    "1a25c",
  ),
  t(
    "list of list",
...
let routes = {
  let fixed = n => regex(Re.(repn(any, n, Some(n))));
  let f = (n, x) => (n, x);
  route([
...
let troute = (title, s, n, res) => (
  ...
  () =>
    A.(check @@ result(pair(int, string), reject))(
      title,
      exec(routes, s),
      [@implicit_arity] Result.Ok(n, res),
    ),
);
